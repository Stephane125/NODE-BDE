...
const boutiqueModel = {
    getAllProducts: (callback) => {
        db.query('SELECT * FROM boutique_test', callback);
    },
    getProductById: (id, callback) => {
        db.query('SELECT * FROM boutique_test WHERE id = ?', id, (error, results) => {
            if (error) {
                callback(error, null);
                return;
            }
    ...
    createProduct: (Name, picture, callback) => {
        db.query('INSERT INTO boutique_test (Name, picture) VALUES (?, ?)', [Name, picture], callback);
    },
    updateProduct: (id, Name, picture, callback) => {
        db.query('UPDATE boutique_test SET Name = ?, picture = ? WHERE id = ?', [Name, picture, id], callback);
    },
    deleteProduct: (id, callback) => {
        db.query('DELETE FROM boutique_test WHERE id = ?', id, callback);
    },
